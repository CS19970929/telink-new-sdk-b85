# 本分支补充：HS-D011 / TLSR8251 / SH3673510

当前 `feature/sh3673510-d011-bms` 的产品目标为 **HS-D011-10S50A-V1 + TLSR8251F512ET32 + SH3673510，默认 10S**。
本段在本分支覆盖下文 D008/DVC1124 专属板型、GPIO、串数与器件分层默认值；其余固定工具链、SDK、存储兼容、错误传播与验证约束仍适用。
先阅读 [D011 硬件事实](docs/D011_HARDWARE_REFERENCE.md) 与 [D011 开发状态/未完成项](docs/D011_DEVELOPMENT_STATUS.md)。
D011 适配位于 `sh3673510_project_config.h`、`sh3673510_control.*`、`sh3673510_bms.c`，既有共用传输仍名为 `sh3673520.*`。
原图 RN3/RN4、加热/F1、MOS 请求与低功耗存在未关闭项；代码存在或 CI 成功不表示硬件已验收。
不得把下文 D008 引脚表当作 D011 连接，也不得由文件名推断 SH3673510/SH3673520 全部寄存器兼容。
