...
| 模块 | 职责 | 不应承担 |
|---|---|---|
| `bms_afe.h` | AFE 初始化、采样、休眠、FET、输出门控和辅助测量契约 | 器件寄存器、协议、全局业务状态 |
...
| `bms_state.h` | 共享运行状态的唯一声明入口 | 任意模块写所有状态 |
| `bms_error.h` | 跨模块错误语义 | AFE 寄存器位定义 |
...
- `g_stCellInfoReport`：对外测量和故障报告；唯一实体由应用层持有。
