...
- **当前 D008 主开发分支：`refactor/d008-bms-phase2`。**
- 旧名 `feature/sh3673510-d013-bmsdvc` 仅作为迁移别名，不再继续开发；后续 D008 修改统一进入 `refactor/d008-bms-phase2`。

分支命名已经与产品身份对齐：**本分支产品硬件是 D008 + DVC1124-2**。D011 和 D013 分别保留自己的 SH3673510 产品分支，不再通过 D008 分支名混用产品代号。
