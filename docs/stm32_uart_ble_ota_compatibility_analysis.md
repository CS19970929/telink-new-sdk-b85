...
**兼容性：可实现。** 当前上位机已经有串口蓝牙模块的 Nordic UART 连接、通知和 Modbus 基础，复用这些代码即可新增 STM32 OTA 后端。

**仅靠上位机：不能保证升级安全。** 上位机可以保证固件类型选择、帧 CRC、发送顺序、每帧 ACK 和重连验证，但无法在 IAP 不支持的情况下凭空获得整包完整性、真实性和断电原子性。

**建议下一步：** 先用一份明确链接到 `0x08001C00` 的 STM32 APP BIN 对现有例程做“兼容模式”链路验证；链路跑通后，再修改 App/IAP 为带会话、序号、镜像长度、整包摘要和严格 Flash 错误处理的正式协议，最后接入当前上位机的自动目标识别和双版本发布流程。
