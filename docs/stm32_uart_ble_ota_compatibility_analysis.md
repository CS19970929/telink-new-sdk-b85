...
4. 将 APP BIN 按 1024 字节补 `0xFF`，每页构造 `0xFFFE` 写多个寄存器帧；完整 1033 字节帧不能依赖 MTU 247 一次写入，而应按 20 字节 BLE 分片并在分片间留出 19200 波特率的串口排空时间；
...
- 透明串口兼容模式固定使用 20 字节 BLE 分片，并对每个分片使用带响应写入；分片后的等待时间按 UART 19200、每字节 10 bit 和 4 ms 安全余量动态计算，若 BLE 带响应写入已经覆盖排空时间则不重复等待；控制帧同样使用带响应写入；
