...
1. 使用与既有 Runtime/PM 相同的 SDK `pm_get_32k_tick()` 时间源，按工程既有 32000 ticks/s 换算，无符号相减处理回绕。积分按 mA × elapsed ticks / (32000 × 100) 得到 As×10，64 位中间值保留余数；校准按实际时间累计 200 ms 策略片，而非按主循环次数。实际时钟精度仍需板测。
