...
│   │       └── ModbusRtu.cs            升级后版本复核用的最小 Modbus RTU codec
...
4. 点击"开始 OTA"。流程：连接 → 发现 OTA 服务 → 订阅通知 → MTU/PDU 协商 → 版本协商 → Start → 有界窗口传输（默认并发 6）→ End → 等待设备 Result → 设备重启 → 自动重连 → 版本复核（OTA 协商 + 尽力读取 BMS 软件版本寄存器 0xC022）→ 判定成功。
