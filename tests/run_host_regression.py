...
def main():
    ...
    product_check = 'sh3673510_d013_diagnostics_integration_check.py'
    if not (ROOT / 'tests' / product_check).is_file():
        raise RuntimeError('Required product contract missing: ' + product_check)
    checks = [p for p in checks if p.name != 'sh3673510_d011_integration_check.py']
    print('Product contract: ' + product_check + '; historical D011 board contract excluded')
    if not checks:
