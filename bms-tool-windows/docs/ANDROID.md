...
- 概览页提供通用“设置当前 SOC”：写公共寄存器 `0x1005`，限制为 `0..100`，写入后读取实时快照核对；不依赖 D008 capability。当前 D008/D011/D013/D014 固件均实现该寄存器，旧固件不支持时必须显示通信失败，不能伪报成功。
...
- 支持 D008 额定容量、循环次数、加热启用/启动温度/停止温度；通用 SOC 设置位于概览页，不受 D008 参数门禁限制。
...
- “SOC 自动测试”连续采样 10 次并检查范围、容量关系、Profile、ETA、Build ID 与跳变；“诊断一致性测试”连续采集 3 次完整诊断并检查 Snapshot、Trace、Errors 与 Build ID。两者复用 `BmsTestEngine`，结果自动保存到 App Documents 目录的 JSON 报告。
