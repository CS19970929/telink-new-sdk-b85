...
- 连接测试报告保存逐轮 UTC/错误、失败类型汇总以及 min/P50/P95/max 端到端耗时；`--output` 可直接交给 AI/CI 留档。
...
- compare 差异已分为 `identity / configuration / runtime`，支持 `--scope` 过滤和 Markdown 报告。
- `bms-cli capabilities` 从共享 `ProductSupportMatrix` 输出 D008/D011/D013/D014 的 AFE、Diagnostics、Runtime、参数协议与证据边界。
...
- 连接 soak 继续增加 RSSI、连接阶段、GATT status、首帧耗时和断开释放结果；端到端分位统计已经实现。
- Windows/Android 显示保护、MOS、Storage 与事件的统一时间线。
- compare 后续增加可配置的动态字段容差；分组、scope 过滤与 Markdown 已实现。
...
- D008/D011/D013/D014 按 capability 选择测试，不通过广播名或分支名猜测产品。
