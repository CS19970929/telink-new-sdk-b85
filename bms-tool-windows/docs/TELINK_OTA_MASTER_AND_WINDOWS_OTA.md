...
2. 用户填写的目标软件版本和升级后版本一致；或
3. 软件版本确实发生变化；或
4. D008 Firmware Build ID 在升级前后发生变化。
...
13. OTA_RESULT 丢失且版本/Build ID 都未改变，必须显示“未完全确认”，不能显示成功。
14. 升级后实时数据、保护参数、SOC、事件日志仍正常。
15. 冷启动多次。
16. 至少两种 Windows BLE 适配器。
