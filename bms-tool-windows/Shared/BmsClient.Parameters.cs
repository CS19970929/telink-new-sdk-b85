...
public sealed partial class BmsClient
{
    public async Task<D008ParameterCapture> ReadD008ParametersAsync(CancellationToken ct=default)
    {
        ...
        try {
            ...
            if(header[0]!=D008Parameters.Magic || header[1]!=D008Parameters.Schema) {
                c.Status="固件不支持 D008 参数协议 v1，仅备份已有参数";
            }
            else c.Supported=true;
    ...
    private async Task RequireD008Async(CancellationToken ct)
    {
        ...
        if(h[0]!=D008Parameters.Magic || h[1]!=D008Parameters.Schema)
            throw new NotSupportedException("设备未声明 D008 参数协议 v1，未发送写命令。");
    }
