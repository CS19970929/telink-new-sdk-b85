...
sealed class FakeTransport:IBmsTransport
{
    ...
    public bool Legacy,Unstable,FailEvents,Timeout;
    ...
    public Task WriteAsync(ReadOnlyMemory<byte> data,CancellationToken ct=default)
    {
        ...
        if(ExceptionCode!=0 || (FailEvents&&start==0xC008)) {Emit(ModbusRtu.Frame(new byte[]{1,0x83,ExceptionCode==0?(byte)2:ExceptionCode}));return Task.CompletedTask;}
        ...
        for(int i=0;i<count;i++) {int at=start-0x2A00+i;ushort value=at>=0&&at<w.Length?w[at]:(ushort)0;BinaryPrimitives.WriteUInt16BigEndian(body.AsSpan(3+i*2,2),value);}
