#!/usr/bin/env python3
"""Run cppcheck and BMS source-contract checks for the 825x ble_sample project."""
...
def has_ordered_tokens(text: str, tokens: Iterable[str]) -> bool:
    start = 0
    for token in tokens:
...
def run_contract_checks() -> list[ContractCheck]:
    bms_c = read_text(BMS_ROOT / "bms_mcu_selftest.c")
    main_c = read_text(BMS_ROOT / "main.c")
    startup_s = read_text(BOOT_ROOT / "cstartup_825x.S")
    boot_subdir = read_text(PROJECT_ROOT / "boot" / "B85" / "subdir.mk")
    vendor_subdir = read_text(PROJECT_ROOT / "vendor" / "ble_sample" / "subdir.mk")
    ...
    startup_order = (
        "BMS_STARTUP_CPU_REG_TEST",
        "BMS_STARTUP_PC_TEST",
        "BMS_STARTUP_RAM_TEST",
        "BMS_STARTUP_FLASH_TEST",
        "BMS_STARTUP_CLOCK_TEST",
        ".section .bss",
    )

    return [
        ContractCheck(
            "SSR-MCU-001 启动期自检位于 C 初始化前",
            has_ordered_tokens(startup_s, startup_order),
            "检查启动汇编中 CPU/PC/RAM/Flash/Clock 自检顺序，并确认其早于 .bss/.data 初始化。",
        ),
        ContractCheck(
            "SSR-MCU-001 启动期自检宏在 B85 工程启用",
            "-DBMS_MCU_STARTUP_SELFTEST_ENABLE=1" in boot_subdir,
            "检查 825x_ble_sample boot/B85/subdir.mk 的汇编宏配置。",
        ),
        ContractCheck(
            "SSR-MCU-002 主循环每轮调用运行期自检",
            "while (1)" in main_c and "bms_mcu_selftest_runtime_check();" in main_c,
            "检查 main 死循环中存在 bms_mcu_selftest_runtime_check 调用。",
        ),
        ContractCheck(
            "SSR-MCU-002 运行期自检顺序符合需求",
            has_ordered_tokens(bms_c, runtime_order),
            "检查 CPU/PC/Clock/Flash/RAM/ADC/IRQ 检查顺序。",
        ),
        ContractCheck(
            "失败路径进入 fail-safe 循环",
            "bms_mcu_selftest_fail_safe_loop(reason);" in bms_c
            and "while (1)" in bms_c
            and "bms_mcu_selftest_force_outputs_off();" in bms_c,
            "检查失败处理不可返回、关断输出并进入报警循环。",
        ),
        ContractCheck(
            "bms_mcu_selftest.c 纳入 B85 ble_sample 构建",
            "bms_mcu_selftest.c" in vendor_subdir and "bms_mcu_selftest.o" in vendor_subdir,
            "检查 825x_ble_sample/vendor/ble_sample/subdir.mk 的源文件和对象文件列表。",
        ),
    ]
...
def write_report(
    ...
    for check in checks:
        lines.append(
            markdown_table_row(
                [check.item, "PASS" if check.passed else "FAIL", check.detail]
            )
        )
    ...
    if failed_checks:
        lines.extend(["", "## 结论", "", "SSR-MCU 契约检查存在失败项，需要修复后复测。"])
    else:
        lines.extend(["", "## 结论", "", "SSR-MCU 契约检查通过；cppcheck 结果见问题明细。"])
...
def run_report() -> int:
    ...
    for check in checks:
        print(f"{'PASS' if check.passed else 'FAIL'}: {check.item}")
