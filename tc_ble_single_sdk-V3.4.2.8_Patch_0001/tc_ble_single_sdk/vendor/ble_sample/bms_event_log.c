...
/*
 * Keep the event-log persistence compatible with the previously proven
 * D008 implementation. Config/state storage remains on Storage V1; the event
 * log intentionally keeps its own append-only snapshot journal so the legacy
 * upper-computer read path at 0xC008 continues to see the same register data.
 */
#define BMS_EVENT_LOG_MAGIC           0x424C4F47u /* BLOG */
#define BMS_EVENT_LOG_VERSION         0x0001u
#define BMS_EVENT_LOG_COMMIT          0x434D4954u /* CMIT */
...
static u32 bms_event_log_flash_base(void)
{
    return flash_store_cfg_get_event_log_base();
}

static u16 bms_event_log_flash_sectors(void)
{
    return flash_store_cfg_get_event_log_sectors();
}
...
{
    u32 base = bms_event_log_flash_base();
    u32 addr;

    if (base == 0u || sector_idx >= bms_event_log_flash_sectors()) return 0;
    addr = base + ((u32)sector_idx * FLASH_SECTOR_SIZE);
    return flash_store_erase_sector_checked(addr, FLASH_SECTOR_SIZE);
}

static u16 bms_event_log_slots_per_sector(void)
{
    return (u16)(FLASH_SECTOR_SIZE / BMS_EVENT_LOG_SNAPSHOT_BYTES);
}
...
static void bms_event_log_build_snapshot(u8 *buf, u32 seq)
{
    ...
    memcpy(&buf[BMS_EVENT_LOG_RECORDS_OFF],
           g_bms_event_log.records,
           sizeof(g_bms_event_log.records));
...
static int bms_event_log_parse_snapshot(u16 slot_idx,
                                        u32 *seq_out,
                                        u16 *write_pos_out,
                                        u8 records_out[BMS_EVENT_LOG_ENTRY_COUNT][2])
{
...
static u16 bms_event_log_prepare_next_slot(void)
{
    ...
    if (current_slot == BMS_EVENT_LOG_INVALID_SLOT) {
        if (!bms_event_log_flash_erase_sector(0u)) return BMS_EVENT_LOG_INVALID_SLOT;
        return 0u;
    }

    next_slot = (u16)(current_slot + 1u);
    if (next_slot >= total_slots) next_slot = 0u;
    ...
    if (next_sector != current_sector) {
        if (!bms_event_log_flash_erase_sector(next_sector)) return BMS_EVENT_LOG_INVALID_SLOT;
        return bms_event_log_sector_first_slot(next_sector);
    }

    if (bms_event_log_slot_is_erased(next_slot)) return next_slot;

    next_sector = (u16)(next_sector + 1u);
    if (next_sector >= bms_event_log_flash_sectors()) next_sector = 0u;
    if (!bms_event_log_flash_erase_sector(next_sector)) return BMS_EVENT_LOG_INVALID_SLOT;
    return bms_event_log_sector_first_slot(next_sector);
}
...
static int bms_event_log_write_snapshot(void)
{
    ...
    if (!bms_event_log_flash_prog(bms_event_log_slot_addr(slot_idx),
                                  buf,
                                  BMS_EVENT_LOG_COMMIT_OFF) ||
        !bms_event_log_flash_prog(bms_event_log_slot_addr(slot_idx) + BMS_EVENT_LOG_COMMIT_OFF,
                                  commit,
                                  sizeof(commit))) {
        bms_event_log_report_store_error();
        return 0;
    ...
    if (g_bms_event_log.next_seq == 0u) g_bms_event_log.next_seq = 1u;
    return 1;
}
...
int bms_event_log_init(void)
{
    ...
    if ((bms_event_log_flash_base() == 0u) ||
        ...
        (bms_event_log_slots_per_sector() == 0u)) {
        bms_event_log_report_store_error();
        return 0;
    }
    ...
    for (slot_idx = 0u; slot_idx < total_slots; ++slot_idx) {
        if (!bms_event_log_parse_snapshot(slot_idx, &seq, &write_pos, records)) continue;
        if ((best_slot == BMS_EVENT_LOG_INVALID_SLOT) || ((int32_t)(seq - best_seq) > 0)) {
            best_slot = slot_idx;
            best_seq = seq;
    ...
    if (best_slot != BMS_EVENT_LOG_INVALID_SLOT) {
        memcpy(g_bms_event_log.records,
               best_records,
               sizeof(g_bms_event_log.records));
...
{
    bms_event_log_track_edge(1u, BALANCE_OPEN);
}
