...
// ===== pins =====
#define PIN_OWC_RX   GPIO_PC0
#define PIN_OWC_TX   GPIO_PC1

// ===== params =====
#define RX_HIGH_STABLE_MS       50
#define UART_DETECT_WINDOW_MS   30
#define UART_FALL_MIN_CNT       3
#define UART_IDLE_BACK_MS       30000   // 0 关闭回退
...
{
    gpio_set_func(PIN_OWC_TX, AS_UART);
    gpio_set_func(PIN_OWC_RX, AS_UART);
    gpio_setup_up_down_resistor(PIN_OWC_RX, PM_PIN_PULLUP_10K);
}
...
static void enter_uart_modbus(void)
{
    ...
    uart_pin_mux_enable();
    // uart_init(115200); // 你工程里如果没初始化过，务必加
...
_attribute_ram_code_ void bus_mux_irq_handler(void)
{
    ...
    if(reg_irq_src & FLD_IRQ_GPIO_RISC0_EN){
        ...
        if(g_state == BUS_STATE_OWC_IDLE || g_state == BUS_STATE_OWC_TX){
            ...
            if(exceed_ms(g_win_start, UART_DETECT_WINDOW_MS)){
                g_win_start = now;
                g_fall_cnt = 0;
            }
            g_fall_cnt++;
...
void bus_mux_task(void)
{
    ...
    if(g_state == BUS_STATE_OWC_IDLE){
        if(rx_is_high()){
            if(g_rx_high_since == 0) g_rx_high_since = tick_now();
            if(exceed_ms(g_rx_high_since, RX_HIGH_STABLE_MS)){
                enter_owc_tx();
                return;
            }
        }else{
    ...
    if(g_state == BUS_STATE_UART_MODBUS && UART_IDLE_BACK_MS > 0){
        if(exceed_ms(g_last_uart_rx, UART_IDLE_BACK_MS)){
            enter_owc_idle();
            return;
        }
    }
