...
void bms_afe_sample(void)
{
    ...
    /* Backends clear AFE1 on a good sample. Remember an actual prior failure
     * until the existing three-sample recovery qualification has completed. */
    ...
    {
        /* Hold a genuine fault through recovery; healthy boot qualification
         * inhibits outputs without fabricating a customer-visible AFE1 error. */
        if (!bms_error_get(BMS_ERROR_AFE1)) bms_error_raise(BMS_ERROR_AFE1);
    }
