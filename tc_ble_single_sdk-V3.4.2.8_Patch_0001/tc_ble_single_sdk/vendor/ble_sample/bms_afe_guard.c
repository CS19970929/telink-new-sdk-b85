...
void bms_afe_init(void)
{
    ...
    s_guard.comm_inhibit = 1u;
    AFE_INIT(); AFE_OUTPUT(0u); (void)AFE_FETS(0u,0u);
    bms_features_init();
    (void)AFE_BAL_SET(0u);
}
...
void bms_afe_sample(void)
{
    ...
    AFE_SAMPLE();
    memset(&m,0,sizeof(m));
    if (!AFE_AUX(&m)) { note_invalid(); return; }
    s_guard.comm_failures = 0u;
    if (s_guard.valid_snapshot_streak < BMS_AFE_VALID_SNAPSHOT_RELEASE_COUNT) ++s_guard.valid_snapshot_streak;
    if (s_guard.valid_snapshot_streak >= BMS_AFE_VALID_SNAPSHOT_RELEASE_COUNT) s_guard.comm_inhibit = 0u;
