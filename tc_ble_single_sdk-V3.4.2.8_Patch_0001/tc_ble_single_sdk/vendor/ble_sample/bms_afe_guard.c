...
#endif
static uint8_t apply_requested(void){uint8_t c=s_guard.requested_charge_on,d=s_guard.requested_discharge_on;if(!s_guard.output_enabled||s_guard.comm_inhibit){c=0u;d=0u;}if(bms_features_charge_blocked())c=0u;if(bms_features_discharge_blocked())d=0u;return AFE_FETS(c,d);}
