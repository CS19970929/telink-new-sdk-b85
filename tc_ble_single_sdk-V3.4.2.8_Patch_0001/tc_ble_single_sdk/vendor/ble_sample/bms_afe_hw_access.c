...
static u16 access_u16be(const u8 *p) { return (u16)(((u16)p[0] << 8) | p[1]); }
...
static int access_response(u8 addr, u8 command, u8 status,
                           ...
                           u8 *rsp, u32 *rsp_len)
{
    ...
    rsp[length++] = command;
    rsp[length++] = status;
    if (payload != 0) for (i = 0u; i < payload_len; ++i) rsp[length++] = payload[i];
...
int bms_afe_hw_access_modbus_on_frame(const u8 *req, u32 req_len,
                                      u8 *rsp, u32 *rsp_len)
{
    u16 crc_rx;
    u16 crc_calc;
    ...
    *rsp_len = 0u;
    ...
    if (command == BMS_AFE_HW_ACCESS_CMD_OPEN)
    {
        ...
        return access_response(req[0], command, BMS_AFE_HW_ACCESS_STATUS_OK, payload, 7u, rsp, rsp_len);
    }
    ...
    case BMS_AFE_HW_ACCESS_CMD_STATUS:
        ...
        return access_response(req[0], command, BMS_AFE_HW_ACCESS_STATUS_OK, payload, 4u, rsp, rsp_len);
    case BMS_AFE_HW_ACCESS_CMD_CLOSE:
        bms_afe_hw_access_close();
        return access_response(req[0], command, BMS_AFE_HW_ACCESS_STATUS_OK, 0, 0u, rsp, rsp_len);
    default:
        return access_response(req[0], command, BMS_AFE_HW_ACCESS_STATUS_UNSUPPORTED, 0, 0u, rsp, rsp_len);
    }
}
