...
static inline void _tl_flash_read(u32 addr, u32 len, u8 *buf){
    // Telink 常见：flash_read_page(addr, len, buf);
    flash_read_page(addr, len, buf);
}

static inline void _tl_flash_write(u32 addr, u32 len, const u8 *buf){
    // Telink 常见：flash_write_page(addr, len, buf);
    flash_write_page(addr, len, buf);
}

static inline void _tl_flash_erase_sector(u32 sector_addr){
 ...
 */
static inline u32 _page_remain(u32 addr){
    u32 off = addr % (u32)NVM_FLASH_PAGE_SIZE;
    return (u32)NVM_FLASH_PAGE_SIZE - off;
}

static inline u32 _min_u32(u32 a, u32 b){ return a < b ? a : b; }
 ...
 * - 每段内部可选进入临界区（建议：短临界区）。
 */
static int telink_flash_read_limited(u32 addr, u8 *buf, u32 len){
    u32 done = 0;
    NVM_LOGT("flash_read req addr=0x%08X len=%u", (unsigned)addr, (unsigned)len);
    while(done < len){
        u32 chunk = _min_u32((u32)NVM_FLASH_MAX_READ_BYTES, len - done);
 ...
 */
static int telink_flash_write_limited(u32 addr, const u8 *buf, u32 len){
    u32 done = 0;
    NVM_LOGT("flash_write req addr=0x%08X len=%u", (unsigned)addr, (unsigned)len);
    while(done < len){
        u32 remain_in_page = _page_remain(addr + done);
        u32 chunk = _min_u32(len - done, remain_in_page);
        // 再按“单次写最大”限制
        while(chunk){
            u32 c2 = _min_u32(chunk, (u32)NVM_FLASH_MAX_WRITE_BYTES);
            FLASH_IF_ENTER_CRITICAL();
            NVM_LOGT("flash_write chunk addr=0x%08X len=%u (page_rem=%u)", (unsigned)(addr + done), (unsigned)c2, (unsigned)_page_remain(addr + done));
            _tl_flash_write(addr + done, c2, (u8*)(buf + done));
#if NVM_FLASH_VERIFY_AFTER_WRITE
            {
                u8 vr[128];
                u32 vlen = c2 > sizeof(vr) ? (u32)sizeof(vr) : c2;
 ...
 */
static int telink_flash_erase_sector_checked(u32 addr){
