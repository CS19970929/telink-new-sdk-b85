...
{
    uint8_t present = 0u;
    /* Prefer an AFE-native detector when the backend can prove one (SH C+ ADC).
     * D008 returns unsupported here and falls back to its schematic CHG-IN GPIO. */
...
static void service_heater(const bms_afe_feature_snapshot_t *s)
{
    ...
    /* Heater power-stage temperature/sensor is a fail-safe condition. */
    ...
    /* Unified charge-heating behavior requested for D008/D011/D013:
     * charger present + Tmin < 0C -> heater ON and CHG MOS blocked;
     * while heating, keep CHG blocked until Tmin >= +5C;
     * charger removal / bad temperature / AFE failure -> heater OFF. */
...
static uint8_t openwire_eligible(void)
{
    ...
    /* An existing open-wire fault is intentionally NOT part of major_fault():
     * idle re-tests must be able to clear a repaired sensing harness. */
...
static void service_openwire(void)
{
    ...
    if (s_feature.openwire_active) {
        ...
        if (state == BMS_AFE_DIAG_READY) {
            ...
            /* Keep the last determinate fault latch. A diagnostic transport
             * error must never silently clear a previously proven open wire. */
            s_feature.openwire_active = 0u;
            s_feature.openwire_idle_samples = 0u;
            s_feature.openwire_cooldown_samples = (uint16_t)BMS_OPENWIRE_PERIOD_SAMPLES;
        }
...
static void service_balance(const bms_afe_feature_snapshot_t *s)
{
    ...
    if (s != 0 && s->valid &&
        ...
        /* V1 compatibility: reuse existing first-level cell-delta threshold.
         * A dedicated balance parameter group can replace this later without
         * changing the common state machine or backend API. */
...
void bms_features_on_afe_invalid(void)
{
    ...
    s_feature.openwire_active = 0u;
    s_feature.openwire_idle_samples = 0u;
    /* Preserve a previously determinate open-wire latch across comm failures. */
    publish_balance(0u);
}
