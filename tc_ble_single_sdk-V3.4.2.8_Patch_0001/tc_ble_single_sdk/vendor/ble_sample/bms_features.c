...
{
    return (bms_sw_protection_charge_blocked() || bms_sw_protection_discharge_blocked() ||
...
static void service_balance(const bms_afe_feature_snapshot_t *s)
{
    ...
    if (!bms_afe_set_balance_mask(desired)) {
        ...
        s_feature.balance_requested_mask = 0u; publish_balance(0u); return;
    }
...
uint8_t bms_features_charge_blocked(void) { return (s_feature.heater_on || s_feature.openwire_active) ? 1u : 0u; }
uint8_t bms_features_discharge_blocked(void) { return s_feature.openwire_active ? 1u : 0u; }
uint8_t bms_features_openwire_active(void) { return s_feature.openwire_active; }
void bms_features_get_openwire_result(bms_afe_openwire_result_t *result) { if (result != 0) *result = s_feature.openwire_result; }
