...
- 当前改为 `soc / dsg / cycle` 任意变化即写
- 8 sector 布局满足当前热数据寿命要求

结论：按当前 `100Ah` 默认容量、`10A` 放电、`2A` 充电估算，寿命仍然远够，核心边界变成“不要把持久化值改成更细粒度高频量”。
...
- 根据当前产品假设，进一步改成 `soc / dsg / cycle` 任意变化即写
...
- 明确检查 `soc_kv` 已经去掉节流变量，且为“变化即写”语义

## 6. 寿命结论

按 `100k P/E` 保守估算：

- `soc_kv`：满足使用，当前按“变化即写”重新计算后仍有很大寿命余量
- `cold_kv`：满足使用
- `runtime`：满足使用
- `event_log`：满足使用
